...
def createNewForest():
    ...
    #***Create lake perimeters***
    
    lake_top_width = WIDTH // 5
    lake_bottom_width = (3 * WIDTH) // 5

    lake_top_height = HEIGHT // 5
    lake_bottom_height = (3 * HEIGHT) // 5

    for x in range(WIDTH):
        for y in range(HEIGHT):
            if (lake_top_width, lake_top_height) <= (x, y) <= (lake_bottom_width, lake_bottom_height):
                #***Lake stays the same for every forest
                forest[(x, y)] = WATER  
